                                            ...
                                            template)
{
  ...
  #setups
  {
    ...
    # indicator_sys_category_create_indicators <- data.table(pfcbl_category=character(0),
    #                                                        sys_category=list(0))
    # indicator_sys_category_create_indicators[,
    #                                          `:=`(pfcbl_category="facility",
    #                                               sys_category=list(c("id")))]
    # 
    # indicator_sys_category_create_indicators[,
    #                                          `:=`(pfcbl_category="client",
    #                                               sys_category=list(c("id")))]
    # 
    # indicator_sys_category_create_indicators[,
    #                                          `:=`(pfcbl_category="borrower",
    #                                               sys_category=list(c("id")))]
    # 
    # indicator_sys_category_create_indicators[,
    #                                          `:=`(pfcbl_category="loan",
    #                                               sys_category=list(c("rank_id")))]
          ...
          #Query database          
          {
            ...
            template_db_current_match_data <- poolWithTransaction(pool,function(conn) {
              ...
              matches <- dbGetQuery(conn,glue("
                                    ...
                                    inner join lateral (select (array[0,
                                                                     ids.rsf_program_id,
                                                                     ids.rsf_facility_id,
                                                                     ids.rsf_client_id,
                                                                     ids.rsf_borrower_id,
                                                                     ids.rsf_loan_id])[ids.pfcbl_category_rank] as rsf_pfcbl_id) as parent on true
                                    ...
                                    where ids.created_in_reporting_asof_date <= $1::date
                                    	and ids.{parent_rsf_pfcbl_id_col} = any((select distinct tm.parent_rsf_pfcbl_id from _temp_match tm))
                                    	and ids.pfcbl_category = $3::text
            ...
            #if the template hasn't submitted any data source row IDs, then all are newable.
            #And if we're changing to new templates then don't exclude unmatched rows since they aren't expected to be present
            # if (any(unique(template_db_current_match_data$indicator_sys_category)=="data_source_row_id",na.rm=T)) {
            ...
            # if (!any(unique(template_db_current_match_data$indicator_sys_category)=="data_source_row_id",na.rm=T) &&
            #            any(template_current_lookup_data$indicator_sys_category=="data_source_row_id",na.rm=T)) {
            #   template_current_lookup_data <- template_current_lookup_data[indicator_sys_category != "data_source_row_id"]
            # }
          ...
          #will also read-in "long" data and therefore not have parent-child relationships on individual rows)
          {
            ...
            if (template$template_ids_method=="pfcbl_id" && any(names(template_data)=="SYSID") && !empty(template_data[SYSID < 0])) {
              ...
              create_new_defined[is.na(creation_asof_date),
                                 creation_asof_date:=template$reporting_import$reporting_asof_date]
