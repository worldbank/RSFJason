...
rsf_program_perform_calculations <- function(pool,
                                             ...
                                             SYS_FLAGS_CALCULATE_MANUAL=4,
                                             SYS_FLAGS_CALCULATE_SYSTEM=8,
  ...
  { 
    ...
    calculation_flags <- data.table(rsf_pfcbl_id=numeric(0),
                                    ...
                                    data_check_value=character(0),
                                    data_check_unit=character(0))
  ...
  { 
    ...
    calculations <- calculations[rsf_indicators[,.(indicator_id,
                                                   unit_fx_method,
                                                   unit_fx_source,
                                                   unit_fx_indicator_id,
                                                   data_category,
                                                   data_type,
                                                   indicator_name,
                                                   is_system)],
    ...
    #this is for unit_fx indicators that are setup to calculate the fx-equivalent value of a reported, non-formula metric.  Eg, loan_original_balance_EUR will convert loan_original_balanace.current -> EUR
    unit_fx_quasi_formulas <- rsf_indicators[!is.na(unit_fx_indicator_id) & 
                                             indicator_id %in% calculations[is.na(formula_id),calculate_indicator_id]
                                            ][,
                   .(calculate_indicator_id=indicator_id,
                     calculate_indicator_name=indicator_name,
                     unit_fx_indicator_id)
                   ][rsf_indicators[,
                                    .(indicator_id,
                                      formula=paste0(indicator_name,".current"),
                                      perform_calculation_by_row=FALSE,
                                      formula_grouping_pfcbl_rank=1,
                                      formula_grouping_rsf_id=as.character(NA),
                                      formula_pfcbl_id_categories=as.list(paste0("rsf_",data_category,"_id")),
                                      formula_overwrite=TRUE,
                                      joincondition=as.numeric(NA))],
                     on=.(unit_fx_indicator_id=indicator_id),
                     nomatch=NULL]
    
    calculations[unit_fx_quasi_formulas,
                 `:=`(formula=i.formula,
                      perform_calculation_by_row=i.perform_calculation_by_row,
                      formula_grouping_pfcbl_rank=i.formula_grouping_pfcbl_rank,
                      formula_grouping_rsf_id=i.formula_grouping_rsf_id,
                      formula_pfcbl_id_categories=i.formula_pfcbl_id_categories,
                      formula_overwrite=i.formula_overwrite),
                 on=.(calculate_indicator_id,
                      formula_id=joincondition)]
    
    
    if (!empty(calculations[is.na(formula)])) {
      print(calculations[is.na(formula)])
      #stop("Calculations with missing formulas")
    }
    #!!NOTE!!
    ...
    calculations[is.na(calculate_indicator_currency_unit) == TRUE &
                   sapply(calculations$parameters_dt,function(x) { any(x$parameter_data_type=="currency" & 
                                                                         grepl("\\.unit$|all$|conflicts$",x$parameter_variable)) #current.unit, previous.unit or .all should all have currency unit information
                   }) == TRUE,
  ...
  #Run calculations by group 
  {
    ...
    #computation_groups <- sort(unique(calculations$calculation_group))
  ...
  #Uploads
    ...
    #Create current results to compare current data values and the calculated results
    {
      ...
      current_results[calculations,
                      `:=`(formula_overwrite=i.formula_overwrite,
                           is_system=i.is_system),
                      on=.(indicator_id=calculate_indicator_id,
                           formula_id)]
      ...
      #If a data point flagged as a manual calculation is prompted for calculation, simply omit it:
      #Don't report it.
      #Don't flag it.
      #But do ensure its entry is removed from rsf_data_calculation_evaluations and it is correctly validated.
      current_results[,
                      omit_reporting:=FALSE]
      
      current_results[is.na(current_data_sys_flags)==FALSE & 
                      bitwAnd(current_data_sys_flags,SYS_FLAGS_CALCULATE_MANUAL) == SYS_FLAGS_CALCULATE_MANUAL & 
    ...
    #calculation_flags
    {
      #If we overwrite the current data (OR) we would if it weren't due to a manual setting or overwrite deny setting, then flag that expectation
      ...
      current_results[is_system ==FALSE &                            #not a system indicator
                        ...
                        (current_data_is_system_calculation==FALSE | #current data is user reported (not system reported)
                         current_value_is_user_monitored==TRUE |     #or it's system reported, but user previously reported it, so let's keep an eye on it.
                         (data_changed==TRUE & insert_action==FALSE) #Unless system calculated it and should updated but can't!
                        ),     
                      flag_overwrite:=TRUE]
      ...
      #Undoing an overwrite flag/calculation means to sys_flag the new data point as soft deleted, removing it from rsf_data_current and letting the EXISTING (current) calculation stand.
      ...
      #Undoing a missing flag/calculation means to sys_flag the new data point as soft deleted, removing it from rsf_data_current and letting the MISSING (previous) calculation stand.
      ...
      #Undoing a VS flag/calculation means to sys_flag the old/flagged as CALCUALTE to prompt the calculator to re-calculate it and flag as normal
                                         ...
                                         .(rsf_pfcbl_id,
                                           ...
                                           check_message=paste0("System: {",
                                                               ...
                                                               "{",
                                                                ifelse(is.na(current_data_value),"MISSING",current_data_value),
                                                                flags_current_unit,"}",
                                                               " / Not overwritten because",
                                                               fcase(omit_reporting==TRUE," set to MANUAL REPORTING",
                                                                     omit_reporting==FALSE,paste0(" indicator overwrite=\"",formula_overwrite,"\""))),
                                           data_check_value=data_value,
                                           data_check_unit=fcase(is.na(data_unit),as.character(NA),
                                                                 data_unit=="LCU",entity_local_currency_unit,
                                                                 data_unit!="LCU",data_unit))]
    ...
    #Upload changes and verify validations
    {
      ...
      if (perform.test==TRUE) {
        ...
        flags <- calculation_flags[,.(flags=paste0(
                                   ...
                                   by=.(rsf_pfcbl_id,
                                        indicator_id,
                                        reporting_asof_date)] #Don't need to retain check_data_value/unit for test flags because data should be stored in the message, too
      ...
      #For all the insert/overwrite data we've calculated it *should* generate a new data_id and push the evaluation_id into non-current because that data_id isn't current (removing the old flag, effectively)
      #Unless that old flag was re-assigned to entity_reporting for some reason and the older data_id remains valid....
      #All new calculation flags should be the most current and only one calculator flag should exist on a data point at any given time.
      #So delete all the calculation flags on the current dataset that's just been calculated
      
      #Calculator checks:
      #sys_calculator_failed                       - Can't calculate
      #sys_calculator_vs_reported_calculation      - Calculator disagrees with manual reporting and denied to overwrite (HAS DATA)
      #sys_calculator_overwrote_manual_calculation - Calculator disagrees with manual reporting of same as-of date and did overwrite
      #sys_calculator_vs_missing_calculation       - Calculator disagrees with manual reporting of different as-of date and did overwrite
      ...
      nx <- poolWithTransaction(pool,function(conn) {
        ...
        dbAppendTable(conn,
                      name="_validations",
                      value=unique(current_results[,.(rsf_pfcbl_id,
                                                      indicator_id)]))
        dbExecute(conn,"analyze _validations")
        
        dbExecute(conn,"
        delete from p_rsf.rsf_data_calculation_evaluations dce
        using _validations val
          where val.rsf_pfcbl_id = dce.rsf_pfcbl_id
            and val.indicator_id = dce.indicator_id
            and dce.calculation_asof_date =  $1::date",
        ...
        dbExecute(conn,"
          ...
          using _validations val 
          where val.rsf_pfcbl_id = rdc.rsf_pfcbl_id
            and val.indicator_id = rdc.indicator_id
            and rdc.check_asof_date = $1::date
            and rdc.indicator_check_id = any(select ic.indicator_check_id 
